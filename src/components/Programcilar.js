...
export default function Programcilar() {
  ...
  const oneCikaninIsmi = () => {
    ...
    return enIyiler.find(dev => dev.id === oneCikaninIdsi).isim;
  };

  const stil = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: oneCikaninIdsi ? 'gold' : 'black' // 🤔 kutlarken renk gold'a dönecek
  };
      ...
      <div className='programmers'>
        {
          ...
          enIyiler.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.isim} <button onClick={() => {/* burada dev.id 'yi öne çıkan id'ye atayın */ setOneCikaninIdsi(dev.id)}}>Kutla</button>
            </div>
          )
        }
      </div>
